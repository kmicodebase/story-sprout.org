...
check(funnel.length > 0 && funnel.every((e) => e.properties?.$geoip_disable === true), 'GeoIP enrichment disabled (no city, postcode or lat/long)')
const geo = Object.keys(funnel[0]?.properties ?? {}).filter((k) => /geoip|latitude|longitude|postal|city|region/i.test(k) && k !== '$geoip_disable')
check(geo.length === 0, 'no location properties sent from the client', geo.join(', ') || 'none')
